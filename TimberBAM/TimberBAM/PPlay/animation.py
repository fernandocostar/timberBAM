...
class Animation(gameimage.GameImage):
    ...
    def __init__(self, image_file, total_frames, loop=True):
        ...
        self.width = self.width / total_frames  # The width of each frame
        ...
        #Modified by Anselmo
        #Create a list of subimages

        self.original_frames = []
        self.work_frames = []
        for frame_index in range(total_frames):
            rect = Rect(frame_index * self.width, 0, self.width, self.height)
            self.original_frames.append(self.image.subsurface(rect))
            self.work_frames.append(self.original_frames[frame_index].convert_alpha())
    ...
    def draw(self):
        if(self.drawable):
            ...
            #Modified by Anselmo
            #Correction for rotation around original image center
            image = self.work_frames[self.curr_frame]
            original_image = self.original_frames[self.curr_frame]
            image_center = image.get_rect().center
            original_center =original_image.get_rect().center
            delta_x = image_center[0]-original_center[0]
            delta_y = image_center[1]-original_center[1]

            window.Window.get_screen().blit(
                image,(self.x-delta_x,self.y-delta_y)
                #self.image,
    ...
    def rotate(self, angle):
        for frameIndex in range(0, self.total_frames):
            self.work_frames[frameIndex] = pygame.transform.rotate(self.original_frames[frameIndex], angle)

    def scale(self, scale):
        for frameIndex in range(0, self.total_frames):
            self.work_frames[frameIndex] = pygame.transform.scale(self.original_frames[frameIndex],
                                                                  (int(self.width*scale),int(self.height*scale)))

    def rotatescale(self,angle,scale):
        for frameIndex in range (0,self.total_frames):
            temp_image = pygame.transform.rotate(self.original_frames[frameIndex],angle)
            self.work_frames[frameIndex] = pygame.transform.scale(temp_image,
                                                                  (int(temp_image.get_rect().width*scale),
                                                                   int(temp_image.get_rect().height*scale)))


    def rotatesmoothscale(self, angle, scale):
        for frameIndex in range (0, self.total_frames):
            temp_image = pygame.transform.rotate(self.original_frames[frameIndex], angle)
            self.work_frames[frameIndex] = pygame.transform.smoothscale(temp_image,
                                                                  (int(temp_image.get_rect().width*scale),
                                                                   int(temp_image.get_rect().height*scale)))
